...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  if (!isReady) {
    return <div className="min-h-[100dvh] bg-[var(--bg-primary)] flex items-center justify-center"><div className="h-12 w-12 rounded-full border-2 border-[rgba(255,255,255,0.08)] border-t-[var(--ikf-red)] animate-spin" /></div>;
  }
    ...
    <main className="relative min-h-[100dvh] overflow-y-auto overflow-x-hidden bg-[var(--bg-primary)] text-[var(--text-primary)]">
      ...
      <section className="relative z-10 grid min-h-[100dvh] grid-cols-1 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="flex items-center justify-center p-4 sm:p-10">
          <motion.form initial={{ opacity: 0, scale: 0.96, y: 18 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.55 }} onSubmit={handleLogin} className="w-full max-w-[470px] overflow-hidden rounded-3xl sm:rounded-[2rem] border border-[rgba(255,255,255,0.1)] bg-[rgba(17,19,24,0.82)] shadow-[0_30px_100px_rgba(0,0,0,0.55)] backdrop-blur-2xl">
            <div className="relative p-6 sm:p-10">
              <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-[var(--ikf-red)] via-[var(--ikf-gold)] to-[var(--corner-blue)]" />
              <div className="mb-7 sm:mb-9 text-center"><div className="mx-auto mb-5 flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-3xl border border-[rgba(200,16,46,0.35)] bg-[rgba(200,16,46,0.1)] shadow-[0_0_60px_rgba(200,16,46,0.22)]"><LockKeyhole className="text-[var(--ikf-red)]" size={30} /></div><h2 className="mt-3 font-display text-4xl sm:text-5xl tracking-wide text-white">Welcome Back</h2><p className="mt-3 text-sm text-[var(--text-secondary)]">Authorized users can access the platform.</p></div>
              <div className="space-y-5">
                <label className="block"><span className="mb-2 block text-xs font-black uppercase tracking-[0.22em] text-[var(--text-muted)]">Login</span><input value={username} onChange={(event) => setUsername(event.target.value)} className="h-14 w-full rounded-2xl border border-[rgba(255,255,255,0.08)] bg-black/25 px-5 text-base font-semibold text-white outline-none transition-all placeholder:text-[var(--text-muted)] focus:border-[var(--ikf-red)] focus:shadow-[0_0_0_4px_rgba(200,16,46,0.12)]" placeholder="Username" autoComplete="username" required /></label>

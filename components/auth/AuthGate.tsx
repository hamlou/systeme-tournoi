    ...
    <main className="relative min-h-screen overflow-hidden bg-[var(--bg-primary)] text-[var(--text-primary)]">
      ...
      <section className="relative z-10 grid min-h-screen grid-cols-1 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="hidden lg:flex flex-col justify-between p-12 xl:p-16">
          ...
          <div className="grid max-w-2xl grid-cols-3 gap-4">{["7 Admins", "Live Sync", "Secure Gate"].map((item) => <div key={item} className="rounded-2xl border border-[rgba(255,255,255,0.07)] bg-white/[0.03] p-4 backdrop-blur-xl"><p className="font-display text-2xl text-white">{item}</p><div className="mt-3 h-1 rounded-full bg-gradient-to-r from-[var(--ikf-red)] to-[var(--ikf-gold)]" /></div>)}</div>
        </div>
